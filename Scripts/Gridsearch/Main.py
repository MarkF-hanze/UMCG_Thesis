...
class BaseAlg():
    """
    Base gridsearch class. This one assumes a sklearn Object. If other thing are used overwrite according methods.
    """
    ...
    def set_classes(self, X):
        """
        Set the classes of the current run
        input variables:
            X: Train data
        """
    ...
    def check_params(self, parameter):
        """
        Check the current gridsearch parameters if they exist in the model
        input variables:
            parameter: Parameters to check
        """
    ...
    def helper_set(self, parameter):
        """
        Set the current gridsearch parameters
        input variables:
            parameter: Parameters to check
        """
    ...
    def set_params(self, parameter):
        ...
        Set the current gridsearch parameters
        input variables:
            parameter: Parameters to check (Dictionary)
        """
...
# Hierarchical clustering need a change in the fit method for faster loading
class Hierachical(BaseAlg):
    """
    Gridsearch for hierarchical clustering (Not used R version is used instead)
    """
    def __init__(self, X):
...
# MAFIA is a tested algorithm that needed to overwrite everything
class MAFIA(BaseAlg):
    def __init__(self):
        """
        Gridsearch for MAFIA (Not used in report)
        """
    ...
    def set_classes(self, X):
        ...
        for filename in os.listdir(directory):
            if filename.endswith(".idx"):
                loadedrow = pd.read_table(f"{directory}/{filename}", sep="  ", header=None,
    ...
    def fit(self, X):
        ...
        start_time = time.time()
        subprocess.run(
            f"/home/g0017139/UMCG_Thesis/Working_Code/bin/cppmafia /data/g0017139/MAFIA/X.dat -a {self.current_params['a']} -b {self.current_params['b']} -n {self.current_params['n']} -u {self.current_params['u']} -M {self.current_params['M']}",
            shell=True)
        fit_time = time.time() - start_time
        self.set_classes(X)
        return fit_time
...
# Elasticsspace subnet basically sklearn object with some small changes
class ENSC(BaseAlg):
    """
    Gridsearch for Elasticsspace (Not used)
    """
    def __init__(self):
...
# Gridsearch with a BaseAlg object or the alterations
class Gridsearch():
    ...
    def get_str(self, alg):
        """
        Get the name of the file to save it correctly with the name being the parameters used for this run
        input parameters:
            alg: current algorithm object
        returns:
            str containing the name of the algorithm + parameters checked
        """
        the_str = str(alg)
    ...
    def fit(self, parameter, folder):
        """
        Fit the algorithm with a set of parameters
        input parameters:
            parameter: parameters to check the accuracy
            folder: str to save the results
        returns:
            The scores of the current set of parameters
        """
        # Fit it
        self.algorithm.set_params(parameter)
    ...
    def start(self, folder):
        ...
        input parameters:
            folder: str to save the results
        returns:
            The scores of the all set of parameters
        """
    ...
    def get_alg(self, name):
        """
        Get the class of the specified algorithm
        input parameters:
            name: str name of the algorithms
        returns:
        ...
        if name == 'kmeans':
            ...
            raise ValueError(
                f'Algorithm {name} is not implemented chose "kmeans" "essc" "ensc" "dbscan" "MAFIA", "hierach"')
        return class_alg
    ...
    def get_score(self, labels):
        ...
        input parameters:
            labels: list of labels assigned by the current algorithm
        returns:
           Dictionary of Silhouette score with the euclidean and correlation distance
        """
        result = {}
        try:
    ...
    def product_dict(self, **kwargs):
        """
        Get all possible combinations of variables in kwargs
        """
        keys = kwargs.keys()
        vals = kwargs.values()
