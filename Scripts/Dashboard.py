...
def make_countplot(df, x, hue, pallete):
    ...
    input variables:
        ...
        x: column in df containing the values
        hue: column in df containing the hues
        pallet: color pallet for the countplot
    return:
        figures containing the countplots
    """
...
def cluster_plot_bokeh(grid, alg):
    ...
    input variables:
        grid: DataFrame of the scores with different parameters
        alg: str algorithm in dictionairy to creaty the plot for
    return:
        Bokeh figure containing the change in scores
    """
    if 'n_clusters' in grid.columns:
        ...
        p.add_tools(
            HoverTool(renderers=[l3], tooltips=[('Silhoette correlation', "@silhouette_score_correlation{0.00}")],
                      mode='vline'))
...
def do_dim_red(X):
    ...
    input variables:
        X: Data to reduce dimensions
    return:
        2 components transformations for both UMAP and PCA
    """
    # Train PCA
...
def make_colors(df):
    ...
    input variables:
        df: DataFrame containing 'type' column
    return:
        Unique colors for every colors type both in a list, mapper, and palette
    """
...
def transform_clusters(clusters):
    ...
    input variables:
        clusters: Cluster labels of a given set of samples
    return:
...
def get_hierarch(load_set, X):
    ...
    input variables:
        load_set: int set to load the results from
        X: ALl samples
    return:
        Cleaned cluster labels and the gridsearch results
    """
...
def transform_results(results):
    """
     Only leave the max silhouette score for every number of clusters
    input variables:
        results: Dictionairy gridserach results
    return:
        Cleaned gridsearch results
    """
...
def tab1(source, df):
    """
    Create the first tab of the dashboard
    input variables:
        source: source containing the different gene expressions and cancer labels
        df: dataframe containing the different gene expressions and cancer labels
    return:
        The first tab of the dashboard
    """
    # Get colors
...
def make_pca_umap(source, colors):
    """
    Create the umap and PCA figures
    input variables:
        source: source containing the different gene expressions and cancer labels
        colors: list of colors to give each sample (HTML colors)
    return:
        Two figures one for PCA and one for UMAP
    """
    # PCA UMAP Plot for
...
def sankey_plot(df, alg):
    """
    Plot the sankey figure, this plot shows how the cluster distribution changed
    input variables:
        df: dataframe containing the different cluster labels for every number of clusters
        alg: str name of the algorithm to create the sankey plot for
    return:
        Sankey plot
    """
        ...
        for x in set(df.iloc[:, i]):
            ...
            for y in set(df.iloc[:, i + 1]):
                ...
                if len(df[((df.iloc[:, i] == x) & (df.iloc[:, i + 1] == y))]) != 0:
                    edges.append(
                        (f'{i}_{x}', f'{i + 1}_{y}', len(df[((df.iloc[:, i] == x) & (df.iloc[:, i + 1] == y))])))
...
def pie_plot(df, color_mapper, alg):
    """
     Plot the pie figures, this plot shows how the cluster cancer types are in each cluster\
    input variables:
         df: dataframe containing the different cluster labels for every number of clusters
         color_mapper: dictionary containing the color for every cancer type
         alg: str name of the algorithm to create the pie plot for
     return:
         pie plot
     """
    tabs_pie = []
...
def heatmap(df, alg):
    """
     Plot the heatmap, used to see which cancer label is in which cluster
    input variables:
         df: dataframe containing the different cluster labels for every number of clusters
         alg: str name of the algorithm to create the pie plot for
     return:
...
def set_board(X, X_with_type, current_set):
    ...
    input variables:
         ...
         X_with_type: Dataframe with gene expression data and cancer label
         current_set: int (1,2,4) Set to load the data from
     return:
         html file with different figures
     """
    # Train PCA and UMAP
    df = do_dim_red(X)
    ...
    df['HierarchicalClustering'] = [cluster_colors[x] for x in
                                    clusters['HierarchicalClustering'][numbers[current_set][0]]]
...
if __name__ == '__main__':
    ...
    for LOADED_SET in range(1, 5):
        if LOADED_SET != 3:
            ...
            set_board(df_normalized, df, LOADED_SET).save(
                f'/home/g0017139/UMCG_Thesis/Scripts/Results/Dashboard_{LOADED_SET}.html')
            plt.clf()
